...
export default function Generator(props) {
  ...
  function handleGenerateWorkout() {
    updateWorkout(); // Update the workout data
    setWorkoutGenerated(true); // Set workout as generated
  }

  // Scroll effect that runs whenever workoutGenerated is true
  useEffect(() => {
    if (workoutGenerated && workoutSectionRef.current) {
      workoutSectionRef.current.scrollIntoView({ behavior: 'smooth' });
    }
  }, [workoutGenerated]); // Trigger scroll when workoutGenerated changes

  return (
    <SectionWrapper id={"generate"} header={"generate your workout"} title={['It\'s', 'Gains', 'Time']}>
      <Header index={"01"} title={"Choose your workout type"} description={"Select the workout type you want to complete:"} />
